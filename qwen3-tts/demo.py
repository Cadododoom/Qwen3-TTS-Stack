...
def load_model(ckpt_name: str) -> str:
    global active_tts, active_ckpt, active_device, active_dtype, active_attn_impl
    try:
        # Load the model
        model = Qwen3TTSModel.from_pretrained(
            ...
            dtype=active_dtype,
            attn_implementation=active_attn_impl,
        )
        active_tts = model
        active_ckpt = ckpt_name
        gc.collect()
        return f"Successfully loaded {ckpt_name}"
    except Exception as e:
        return f"Failed to load {ckpt_name}: {type(e).__name__}: {str(e)}"


def get_choices_for_active_model():
    global active_tts
    supported_langs_raw = None
    if active_tts and callable(getattr(active_tts.model, "get_supported_languages", None)):
        supported_langs_raw = active_tts.model.get_supported_languages()
    lang_choices, lang_mapping = _build_choices_and_map([x for x in (supported_langs_raw or [])])
    if "Auto" not in lang_choices and lang_choices:
        lang_choices = ["Auto"] + lang_choices
    
    supported_spks_raw = None
    if active_tts and callable(getattr(active_tts.model, "get_supported_speakers", None)):
        supported_spks_raw = active_tts.model.get_supported_speakers()
    spk_choices, spk_mapping = _build_choices_and_map([x for x in (supported_spks_raw or [])])
    
    return lang_choices, lang_mapping, spk_choices, spk_mapping


def change_model_event(new_model_name):
    status = load_model(new_model_name)
    if "Failed" in status:
        return status, gr.update(), gr.update(), gr.update(), gr.update(), gr.update()
    
    lang_choices, lang_mapping, spk_choices, spk_mapping = get_choices_for_active_model()
    
    global active_lang_map, active_spk_map
    active_lang_map = lang_mapping
    active_spk_map = spk_mapping
    
    default_lang = "Auto" if "Auto" in lang_choices else (lang_choices[0] if lang_choices else None)
    default_spk = spk_choices[0] if spk_choices else None
    
    return (
        status,
        gr.Dropdown(choices=lang_choices, value=default_lang),
        gr.Dropdown(choices=lang_choices, value=default_lang),
        gr.Dropdown(choices=lang_choices, value=default_lang),
        gr.Dropdown(choices=lang_choices, value=default_lang),
        gr.Dropdown(choices=spk_choices, value=default_spk)
    )


def run_voice_clone(ref_aud, ref_txt: str, use_xvec: bool, text: str, lang_disp: str):
    global active_tts, active_lang_map
    try:
        if active_tts is None:
            return None, "Error: No model loaded."
        mt = getattr(active_tts.model, "tts_model_type", None)
        if mt != "base":
            return None, f"Error: Active model type is '{mt}'. Zero-Shot Voice Cloning requires a 'base' model (e.g. Qwen3-TTS-12Hz-0.6B-Base or Qwen3-TTS-12Hz-1.7B-Base)."
        ...
        language = active_lang_map.get(lang_disp, "Auto")
        wavs, sr = active_tts.generate_voice_clone(
            text=text.strip(),
            language=language,
            ref_audio=at,
...
def run_instruct(text: str, lang_disp: str, spk_disp: str, instruct: str):
    global active_tts, active_lang_map, active_spk_map
    try:
        if active_tts is None:
            return None, "Error: No model loaded."
        mt = getattr(active_tts.model, "tts_model_type", None)
        if mt != "custom_voice":
            return None, f"Error: Active model type is '{mt}'. Custom Voice / Timbre Control requires a 'custom_voice' model (e.g. Qwen3-TTS-12Hz-1.7B-CustomVoice)."
        ...
        language = active_lang_map.get(lang_disp, "Auto")
        speaker = active_spk_map.get(spk_disp, spk_disp)
        wavs, sr = active_tts.generate_custom_voice(
            text=text.strip(),
            language=language,
            speaker=speaker,
...
def run_voice_design(text: str, lang_disp: str, design: str):
    ...
    try:
        ...
        mt = getattr(active_tts.model, "tts_model_type", None)
        if mt != "voice_design":
            return None, f"Error: Active model type is '{mt}'. Voice Design requires a 'voice_design' model (e.g. Qwen3-TTS-12Hz-1.7B-VoiceDesign)."
        ...
        language = active_lang_map.get(lang_disp, "Auto")
        wavs, sr = active_tts.generate_voice_design(
            text=text.strip(),
            language=language,
...
def save_prompt(ref_aud, ref_txt: str, use_xvec: bool):
    ...
    try:
        ...
        mt = getattr(active_tts.model, "tts_model_type", None)
        if mt != "base":
            return None, f"Error: Active model type is '{mt}'. Save Voice requires a 'base' model."
        ...
        items = active_tts.create_voice_clone_prompt(
            ref_audio=at,
            ref_text=(ref_txt.strip() if ref_txt else None),
            x_vector_only_mode=bool(use_xvec),
        )
...
def load_prompt_and_gen(file_obj, text: str, lang_disp: str):
    ...
    try:
        ...
        mt = getattr(active_tts.model, "tts_model_type", None)
        if mt != "base":
            return None, f"Error: Active model type is '{mt}'. Load Voice requires a 'base' model."
...
def build_demo(tts: Qwen3TTSModel, ckpt: str, gen_kwargs_default: Dict[str, Any]) -> gr.Blocks:
    global active_tts, active_ckpt, active_gen_kwargs, active_lang_map, active_spk_map
    active_tts = tts
    active_ckpt = ckpt
    active_gen_kwargs = gen_kwargs_default

    # Initialize choices for initial model
    lang_choices_disp, lang_map = _build_choices_and_map([x for x in (getattr(tts.model, "get_supported_languages", lambda: [])() or [])])
    if "Auto" not in lang_choices_disp and lang_choices_disp:
        lang_choices_disp = ["Auto"] + lang_choices_disp
    spk_choices_disp, spk_map = _build_choices_and_map([x for x in (getattr(tts.model, "get_supported_speakers", lambda: [])() or [])])

    active_lang_map = lang_map
    active_spk_map = spk_map
    ...
    with gr.Blocks(theme=theme, css=css) as demo:
        gr.Markdown(
            """
# Qwen3-TTS Multi-Model Hub (CPU-Optimized)
Select a model from the dropdown below to load it. Model loading takes about 20-30 seconds on CPU.
"""
        )
        ...
        with gr.Row():
            ...
            with gr.Column(scale=3):
                status_box = gr.Textbox(
                    label="Model Status (模型状态)",
                    value=f"Successfully loaded {ckpt}",
                    interactive=False
                )

        with gr.Tabs() as main_tabs:
            with gr.Tab("Zero-Shot Voice Cloning (克隆并合成)"):
                with gr.Tabs():
                    with gr.Tab("Clone & Generate (在线克隆合成)"):
                        with gr.Row():
                            ...
                            with gr.Column(scale=2):
                                text_in_clone = gr.Textbox(
                                    ...
                                    lines=4,
                                    placeholder="Enter text to synthesize (输入要合成 of 文本).",
                                )
                                ...
                                btn_clone = gr.Button("Generate (生成)", variant="primary")

                            with gr.Column(scale=3):
                                audio_out_clone = gr.Audio(label="Output Audio (合成结果)", type="numpy")
                                err_clone = gr.Textbox(label="Status (状态)", lines=2)
...
"""
            ...
            with gr.Tab("Custom Voice / Predefined Timbres (预定义音色控制)"):
                with gr.Row():
                    with gr.Column(scale=2):
                        ...
                        instruct_in_custom = gr.Textbox(
                            label="Instruction (Optional) (控制指令，可不输入)",
                            lines=2,
                            placeholder="e.g. Say it in a very angry tone (例如：用特别伤心的语气说).",
                        )
                        btn_custom = gr.Button("Generate (生成)", variant="primary")
                    with gr.Column(scale=3):
                        audio_out_custom = gr.Audio(label="Output Audio (合成结果)", type="numpy")
                        err_custom = gr.Textbox(label="Status (状态)", lines=2)

            with gr.Tab("Voice Design (自然语言音色描述)"):
                with gr.Row():
                    with gr.Column(scale=2):
                        ...
                        btn_design = gr.Button("Generate (生成)", variant="primary")
                    with gr.Column(scale=3):
                        audio_out_design = gr.Audio(label="Output Audio (合成结果)", type="numpy")
                        err_design = gr.Textbox(label="Status (状态)", lines=2)
...
"""
        ...
        # Bind switching event
        model_selector.change(
            fn=change_model_event,
            inputs=[model_selector],
            outputs=[status_box, lang_in_clone, lang_in2, lang_in_custom, lang_in_design, spk_in_custom]
        )

        # Bind generation handlers
        btn_clone.click(
            fn=run_voice_clone,
            inputs=[ref_audio, ref_text, xvec_only, text_in_clone, lang_in_clone],
        ...
        btn_custom.click(
            fn=run_instruct,
            inputs=[text_in_custom, lang_in_custom, spk_in_custom, instruct_in_custom],
            outputs=[audio_out_custom, err_custom]
        )

        btn_design.click(
            fn=run_voice_design,
            inputs=[text_in_design, lang_in_design, design_in],
...
def main(argv=None) -> int:
    ...
    # Set active settings for dynamic reloading
    global active_device, active_dtype, active_attn_impl
    active_device = args.device
    active_dtype = dtype
    active_attn_impl = attn_impl or "sdpa"

    # Initial load
    status = load_model(ckpt)
    print(status)

    gen_kwargs_default = _collect_gen_kwargs(args)
    demo = build_demo(active_tts, ckpt, gen_kwargs_default)
